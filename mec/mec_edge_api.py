...
POST /telemetry     — validate, score, ML predict, return
GET  /health        — liveness check
GET  /metrics       — Prometheus scrape
GET  /events        — last 50 events log
POST /model/upload  — hot-swap ML model
GET  /explain       — SHAP-style feature importance per vehicle (Week 9)
"""
from flask import Flask, request, jsonify
import time, json, math, logging, collections, copy
import numpy as np
...
# ── Week 9: Feature importance Prometheus gauges ──────────────────────────────
FEATURES = ['rtt_ms', 'packet_loss_percent', 'dl_mbps', 'ul_mbps', 'rsrp_dbm', 'sinr_db']
g_feat_importance = {
    feat: Gauge(
        f'mec_feature_importance_{feat.replace("%","pct")}',
        f'Feature importance for {feat}',
        ['vehicle_id']
    )
...
def anomaly_score(vid, payload):
    ...
    if (payload.get('speed_kmh', 0) == CONFIG["anomaly"]["stopped_speed_threshold"]
            and payload.get('engine_temp_c', 85) > CONFIG["anomaly"]["overheat_temperature_threshold"]):
...
def cleanup_stale_metrics(now=None):
    ...
    for vid in stale:
        ...
        last_importance.pop(vid, None)
        last_payload.pop(vid, None)
        for metric in (g_anomaly, g_proc_ms, g_protocol, g_quality, g_flag, g_attack):
            try: metric.remove(vid)
            except KeyError: pass
        for feat in FEATURES:
            try: g_feat_importance[feat].remove(vid)
            except KeyError: pass
        for metric in (c_tamper, c_valid_ok, c_posts, c_reject):
            try: metric.remove(vid)
            except KeyError: pass
        for attack_type in CONFIG["attack_map"]:
            try: c_attack.remove(vid, attack_type)
            except KeyError: pass

# ── Week 9: Permutation-based feature importance ──────────────────────────────
def compute_feature_importance(payload):
    if attack_model is None or scaler is None:
        return {}
    try:
        row = {feat: float(payload.get(feat, 0)) for feat in ACHANI_FEATURES}
        df_orig = pd.DataFrame([row], columns=ACHANI_FEATURES)
        X_orig  = scaler.transform(df_orig)
        proba_orig    = attack_model.predict_proba(X_orig)[0]
        baseline_conf = float(np.max(proba_orig))
        scaler_mean   = scaler.mean_ if hasattr(scaler, 'mean_') else np.zeros(len(ACHANI_FEATURES))
        importances = {}
        for i, feat in enumerate(ACHANI_FEATURES):
            row_p = row.copy()
            row_p[feat] = float(scaler_mean[i])
            df_p  = pd.DataFrame([row_p], columns=ACHANI_FEATURES)
            X_p   = scaler.transform(df_p)
            perturbed_conf = float(np.max(attack_model.predict_proba(X_p)[0]))
            importances[feat] = max(0.0, round(baseline_conf - perturbed_conf, 4))
        max_imp = max(importances.values()) if importances else 1.0
        if max_imp > 0:
            importances = {k: round(v / max_imp, 4) for k, v in importances.items()}
        return importances
    except Exception as e:
        logging.warning(f"Feature importance error: {e}")
        return {}

# ── Achani ML prediction ──────────────────────────────────────────────────────
...
def recv_telemetry():
    ...
    if errors:
        ...
        return jsonify({
            'status': 'rejected', 'vehicle_id': vid,
            'tamper_detected': True, 'validation_error': errors[0],
            'timestamp': datetime.now(timezone.utc).isoformat(),
        }), 400
    ...
    # Week 9: feature importance
    importance = compute_feature_importance(data)
    if importance:
        last_importance[vid] = importance
        last_payload[vid]    = data
        for feat, imp_val in importance.items():
            g_feat_importance[feat].labels(vehicle_id=vid).set(imp_val)
    log_event(vid, 'OK',
    ...
    return jsonify({
        'status': 'ok', 'vehicle_id': vid,
        'anomaly_score': score, 'tamper_detected': False,
        'validation_passed': True, 'processing_time_ms': proc_ms,
        'timestamp': datetime.now(timezone.utc).isoformat(),
...
def explain():
    ...
    if vid:
        if vid not in last_importance:
            return jsonify({'status': 'no_data', 'vehicle_id': vid,
                            'message': 'No predictions yet for this vehicle'}), 404
        return jsonify({
            'status': 'ok', 'vehicle_id': vid,
            'feature_importance': last_importance[vid],
            'features_ranked': sorted(last_importance[vid].items(), key=lambda x: -x[1]),
            'last_prediction': {f: last_payload[vid].get(f) for f in ACHANI_FEATURES},
            'model_used': 'attack_classifier (RandomForest)',
            'method': 'permutation_importance',
            'timestamp': datetime.now(timezone.utc).isoformat(),
        }), 200
    result = {v: {'feature_importance': last_importance[v],
                  'features_ranked': sorted(last_importance[v].items(), key=lambda x: -x[1])}
              for v in vehicles_seen if v in last_importance}
    return jsonify({'status': 'ok', 'vehicles': result,
                    'method': 'permutation_importance',
                    'model_used': 'attack_classifier (RandomForest)',
...
def health():
    ...
    return jsonify({
        ...
        'feature_importance_enabled': attack_model is not None,
    }), 200
...
def root():
    return jsonify({
        ...
        'unit': 'COMP6016',
        'version': 'COMP6016-v1.0',
        'week9_features': ['permutation_importance', 'attack_timeline', 'feature_importance_grafana'],
        'endpoints': [
            'POST /telemetry',
            'GET  /health',
            'GET  /metrics',
            'GET  /events',
            'POST /model/upload',
            'GET  /explain            (Week 9 - feature importance)',
            'GET  /explain?vehicle_id=MV-001',
        ]
    }), 200
